...
public class LikeCommentServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
//        processRequest(request, response);
String commentIdRaw = request.getParameter("comment_id");
        int userId = 3; // Giả sử user đang đăng nhập

        try {
            int commentId = Integer.parseInt(commentIdRaw);

            Blog_CateDAO dao = new Blog_CateDAO();
            dao.likeComment(commentId, userId);

            // Quay lại trang trước
            String referer = request.getHeader("referer");
            response.sendRedirect(referer != null ? referer : "blogc");
